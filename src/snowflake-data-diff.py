...
def load_sf_db_list(count):
    try:
        ...
        db_name = st.selectbox('Please select the database that you would like to compare?',
                               db_list_df['DATABASE_NAME'], key=f'db_{count + 1}')
        ...
        schema_list_df = pd.DataFrame(schema_list,
                                      columns=['created_on', 'name', 'kind', 'database_name', 'SCHEMA_NAME'])
        schema_name = st.selectbox('Please select the schema that you would like to compare?',
                                   schema_list_df['name'], key=f'schema_{count + 2}')
        ...
        table_list_df = pd.DataFrame(table_list,
                                     columns=['created_on', 'name', 'kind', 'database_name', 'SCHEMA_NAME'])
        table_name = st.selectbox('Please select the table that you would like to compare?',
                                    table_list_df['name'], key=f'table_{count + 3}')
        ...
        key_column_name = st.selectbox('Please select the unique key (primary key)?',
                                       column_list_df['column_name'], key=f'key_{count + 4}')
